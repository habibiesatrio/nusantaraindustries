...
import { CommodityData, CommodityDerivative } from '@/types';
...
const DerivativeTree: React.FC<{ derivatives: CommodityDerivative[], level?: number }> = ({ derivatives, level = 0 }) => {
  return (
    <div className={level > 0 ? 'ml-6' : ''}>
      {derivatives.map((der, i) => (
        <div key={i} className="flex items-start gap-4 mt-4">
          {level > 0 && <div className="w-1.5 h-full bg-blue-100 rounded-full"></div>}
          <div className="flex-1">
            ...
            {der.subProducts && <DerivativeTree derivatives={der.subProducts} level={level + 1} />}
          </div>
        </div>
      ))}
    </div>
          ...
          <div className="bg-white w-full max-w-3xl rounded-[48px] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-500 border border-slate-100">
            ...
            <div className="p-16 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                <DerivativeTree derivatives={selectedCommodity.derivatives} />
              </div>
