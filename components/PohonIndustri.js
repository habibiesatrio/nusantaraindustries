...
import { 
    ...
    Download, Bell, Settings, Database, PlusCircle, 
...
const PohonIndustriContent = () => {
    ...
    const handleSaveData = async (formData) => {
        try {
            ...
            if (modalConfig.mode === 'add') {
                ...
                const position = currentNodes.length > 0 
                    ? { x: currentNodes[0].position.x, y: currentNodes[currentNodes.length - 1].position.y + 350 }
                    : { x: 0, y: 0 };
    ...
    const buildIntegratedTree = useCallback((selectedProduct) => {
        ...
        const getDepth = (id) => {
            ...
            let d = 0, curr = item;
            while(curr?.Parents?.length > 0) { d++; curr = allData.find(x => x.Harmony_ID === curr.Parents[0].Harmony_ID); if (d > 12) break; }
            return d;
        };
        ...
        setEdges(Array.from(edgesMap.values()).map((e, idx) => ({
            id: `e-${idx}`, source: e.source, target: e.target, label: e.label,
    ...
    return (
        <div className="flex h-screen bg-sky-50 font-sans overflow-hidden text-slate-900 leading-none">
            {/* SIDEBAR */}
            <aside className="w-72 bg-white border-r border-slate-100 flex flex-col p-6 sticky top-0 h-screen z-50">
                ...
                <nav className="flex-1 space-y-2 overflow-y-auto custom-scrollbar pr-1">
                    ...
                    <div className="pt-8 px-2 border-t border-slate-50 mt-4">
                        <h2 className="text-[10px] font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center gap-2 font-sans"><Database size={14} className="text-slate-400"/> Navigasi Produk</h2>
                        <div className="relative group">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-sky-500" size={14} />
                            <input type="text" placeholder="Cari HS / Nama..." className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold outline-none focus:ring-2 focus:ring-sky-100" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>
                        <div className="mt-4 space-y-1 max-h-[40vh] overflow-y-auto pr-1">
                            {searchTerm && allData.filter(i => i.Product_Name?.toLowerCase().includes(searchTerm.toLowerCase())).map((item, idx) => (
                                <button key={`${item.Harmony_ID}-${idx}`} onClick={() => buildIntegratedTree(item)} className="w-full text-left p-3 rounded-xl hover:bg-sky-50 group transition-all border border-transparent hover:border-sky-100">
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative bg-white">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-8 bg-white border-b border-slate-100 shadow-sm z-10 font-sans">
                    ...
                    <div className="flex gap-4 font-sans">
                        <button className="bg-white border border-slate-200 p-2.5 rounded-xl text-slate-500 shadow-sm hover:bg-slate-50 transition-all"><Bell size={20}/></button>
                        <button className="bg-white border border-slate-200 p-2.5 rounded-xl text-slate-500 shadow-sm hover:bg-slate-50 transition-all"><Settings size={20}/></button>
                        <button onClick={handleAddNewProduct} className="bg-sky-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all font-sans tracking-wide flex items-center gap-2">
                        ...
                        <button className="bg-sky-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-sky-100 hover:bg-sky-700 transition-all font-sans tracking-wide">
                            <Download size={16} className="inline mr-2"/> Export Report
                        </button>
                    </div>
                </header>
                        ...
                        return (
                            <div className="absolute top-10 right-10 w-[420px] bg-white/95 backdrop-blur-md shadow-[0_25px_60px_rgba(0,0,0,0.2)] rounded-[2.5rem] p-8 border border-slate-100 z-[100] animate-in slide-in-from-right duration-300 font-sans max-h-[85vh] overflow-y-auto custom-scrollbar">
                                <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
                                    <div><p className="text-[9px] font-black text-sky-600 uppercase tracking-[0.2em] mb-1.5">{selectedPopUp.Tier}</p><h3 className="text-xl font-black text-slate-900 uppercase leading-tight tracking-tight">{selectedPopUp.Product_Name}</h3></div>
                                    <button onClick={() => setSelectedPopUp(null)} className="p-2 bg-slate-50 hover:bg-red-50 rounded-full transition-colors group"><X size={20} className="text-slate-300 group-hover:text-red-500" /></button>
                                </div>
            ...
            <DataModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveData} initialData={modalConfig.initialData} mode={modalConfig.mode} allData={allData} />
        </div>
    );
};
